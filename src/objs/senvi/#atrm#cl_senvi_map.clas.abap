CLASS /atrm/cl_senvi_map DEFINITION
  PUBLIC
  CREATE PROTECTED.

  PUBLIC SECTION.
    DATA: senvi  TYPE senvi READ-ONLY,
          origin TYPE REF TO /atrm/cl_object READ-ONLY.
...
CLASS /atrm/cl_senvi_map IMPLEMENTATION.
  ...
  METHOD get.
    ...
    TRY.
      ...
      CATCH cx_dynamic_check.
        CREATE OBJECT map TYPE /atrm/cl_senvi_map
          EXPORTING
            senvi  = senvi
            origin = origin.
    ENDTRY.
  ...
  METHOD sanitize_object_name.
    ...
    IF strlen( raw_copy ) <= max_length.
      sanitized = raw_copy.
    ELSE.
      /atrm/cx_exception=>raise( iv_message = 'Object name ' && raw_copy && ' exceedes max. length ' && max_length ).
    ENDIF.
  ENDMETHOD.
