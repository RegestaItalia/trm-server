CLASS /atrm/cl_post_activity DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
  ...
  PUBLIC SECTION.
    ...
    "! @parameter data     | XML input defining the class and parameters that will be execute
    "! @parameter messages | Table of messages returned from the pre-check
    ...
    "! @parameter data     | XML input defining the class and parameters to execute
...
CLASS /atrm/cl_post_activity IMPLEMENTATION.
  ...
  METHOD run.
    ...
    IF sy-subrc <> 0 OR lo_typedescr IS NOT BOUND.
      /atrm/cx_exception=>raise(
        iv_message = lv_error
        iv_reason  = /atrm/cx_exception=>c_reason-pa_not_found
      ).
    ENDIF.
    ...
    LOOP AT ls_method-parameters INTO ls_method_param.
      ...
      IF sy-subrc <> 0.
        IF ls_method_param-is_optional <> 'X'.
          ...
          /atrm/cx_exception=>raise(
            iv_message = lv_error
            iv_reason  = /atrm/cx_exception=>c_reason-pa_param_missing
          ).
        ELSE.
          CONTINUE.
        ENDIF.
      ...
      IF ls_method_param-parm_kind <> 'I'.
        ...
        /atrm/cx_exception=>raise(
          iv_message = lv_error
          iv_reason  = /atrm/cx_exception=>c_reason-pa_unexpected_param
        ).
      ENDIF.
      CLEAR lo_paramtype.
