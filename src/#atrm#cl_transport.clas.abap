...
CLASS /atrm/cl_transport DEFINITION
  ...
  PUBLIC SECTION.
    ...
    "! @parameter trm_trkorr | Generated TRM transport ID
    "! @raising /atrm/cx_exception | Raised on number range or DB failure
    METHODS migrate
      EXPORTING trm_trkorr TYPE /atrm/trkorr
      RAISING   /atrm/cx_exception.
...
CLASS /atrm/cl_transport IMPLEMENTATION.
  ...
  METHOD migrate.
    ...
    IF sy-subrc <> 0.
      IF sy-subrc EQ 1.
        /atrm/cx_exception=>raise( iv_reason = /atrm/cx_exception=>c_reason-snro_interval_not_found ).
      ELSE.
        /atrm/cx_exception=>raise( ).
      ENDIF.
    ENDIF.
    ...
    IF lt_trm_tmsbuffer[] IS NOT INITIAL.
      /atrm/cl_utilities=>add_migration_tmsbuffer( data = lt_trm_tmsbuffer ).
    ENDIF.
    ...
    IF lt_trm_doktl[] IS NOT INITIAL.
      /atrm/cl_utilities=>add_migration_doktl( data = lt_trm_doktl ).
    ENDIF.
    ...
    IF lt_trm_e071[] IS NOT INITIAL.
      /atrm/cl_utilities=>add_migration_e071( data = lt_trm_e071 ).
    ENDIF.
    ...
    SELECT SINGLE * FROM /atrm/skiptrkorr INTO ls_skip_trkorr WHERE trkorr EQ gv_trkorr.
    IF sy-subrc EQ 0.
      /atrm/cl_utilities=>remove_skip_trkorr( trkorr = gv_trkorr ).
      /atrm/cl_utilities=>add_skip_trkorr( trkorr = lv_trm_trkorr ).
    ENDIF.

    " move src trkorr (if exists)
    SELECT SINGLE * FROM /atrm/src_trkorr INTO ls_src_trkorr WHERE trkorr EQ gv_trkorr.
    IF sy-subrc EQ 0.
      /atrm/cl_utilities=>remove_source_trkorr( trkorr = gv_trkorr ).
      /atrm/cl_utilities=>add_source_trkorr( trkorr = lv_trm_trkorr ).
    ENDIF.
