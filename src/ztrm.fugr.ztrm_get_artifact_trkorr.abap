...
*"     VALUE(IV_VERSION) TYPE  ZTRM_PACKAGE_VERSION
...
*"----------------------------------------------------------------------
  ...
  SELECT SINGLE trkorr trstatus FROM ztrm_trkorr_stat INTO CORRESPONDING FIELDS OF ls_trkorr_stat WHERE package_name EQ iv_name AND version EQ iv_version AND registry EQ iv_registry.
